...
def ord_mezcla(lista):
    """Ordena una lista de elementos según el método de mezcla (merge sort).
       Pre: los elementos de la lista deben ser comparables.
       Post: la lista está ordenada. Devuelve la lista ordenada."""
    if len(lista) < 2:
        lista_nueva = lista
    else:
        medio = len(lista) // 2
...
def experimento_timeit(Nmax):
    ...
    en un vector.
    El parámetro Nmax es la longitud de la lista mas larga ordenada.
    """
    tiempos_sel = []
    tiempos_ins = []
    tiempos_bur = []
...
def graficar(experimento,Nmax):
    '''Grafica un experimento de tiempos de ordenamiento. 
        Pre: Un experimento y la longitud de la lista mas larga ordenada.
        Post: Gráfico.'''
    fig, ax = plt.subplots()
