...
def leer_camion(nombre_archivo):
    with open(nombre_archivo) as f:
        camion = fileparse.parse_csv(f, select = ['nombre', 'cajones', 
                'precio'], types = [str, int, float])
    return camion

def leer_precios(nombre_archivo):
    with open(nombre_archivo) as f:
        lista_precios = fileparse.parse_csv(f, types = [str, float], 
                       has_headers = False)
...
def hacer_informe(lista_cajones, precios):
    ...
    for cajon in lista_cajones:
        cajon_aux = (str(cajon['nombre']), int(cajon['cajones']), 
                   float(precios[cajon['nombre']]), 
                   float(precios[cajon['nombre']] - cajon['precio']))
